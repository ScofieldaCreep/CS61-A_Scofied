...
class Fib():
    ...
    def next(self):     # assign 'value' and 'previous' attributes within 'next'
        "*** YOUR CODE HERE ***"
        if self.value == 0:
            next_fib = Fib(1)
        else:
...
class VendingMachine:
    ...
    def deposit(self, offer):
        if self.amount == 0:
            return 'Machine is out of stock. Here is your ${0}.'.format(offer)
        else:
            self.money += offer
    ...
    def restock(self, num):
        self.amount += num
        return 'Current {0} stock: {1}'.format(self.product, self.amount)  # {0} {1} 是有顺序的
