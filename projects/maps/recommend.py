...
def find_closest(location, centroids):
    ...
    """
    # BEGIN Question 3
    closest_centroid = min(centroids, key=lambda centroid: distance(centroid, location))
...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    return [mean(l) for l in zip(*[restaurant_location(r) for r in cluster])]  # 不理解
...
def find_predictor(user, restaurants, feature_fn):
    ...
    #  s_xy = sum([(x - mean(xs)) * (y - mean(ys)) for x, y in xs and ys])
    #  这句会导致 TypeError: cannot unpack non-iterable int object
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    "*** YOUR CODE HERE ***"
    predictors = {find_predictor(user, reviewed, fn)[0]: find_predictor(user, reviewed, fn)[1] for fn in feature_fns}
...
def rate_all(user, restaurants, feature_fns):
    ...
    for r in restaurants:
        ...
        if r in reviewed:
            rate_list[name] = user_rating(user, name)
        else:
            rate_list[name] = predictor(r)
    return rate_list
    # END Question 9
