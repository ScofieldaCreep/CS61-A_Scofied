...
def flatten(lst):
    ...
    # 简洁版本
    # [1, [2, 3], 4]，先进入else递归，然后从第一个开始，把非list的加[]，list的继续递归，最终得到的全是[]
    if type(lst) != list:
        return [lst]
    else:
...
def put_piece(board, max_rows, column, player):
    ...
    -1
    """
    for i in range(max_rows - 1, -1, -1): # range(x, y, z) x:Start y:Stop z:步长
        if board[i][column] == '-':
...
def print_board(board, max_rows, max_cols):
    ...
    for row in range(max_rows):
        print((''.join(get_piece(board, row, col)+' ' for col in range(max_cols))).strip())
