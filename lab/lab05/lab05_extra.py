...
def prune_leaves(t, vals):
    ...
    if is_leaf(t):
        ...
        new_root = label(t)
        new_branches = [prune_leaves(b, vals) for b in branches(t) if prune_leaves(b, vals) is not None]
...
def sprout_leaves(t, vals):
    ...
    if is_leaf(t):
        ...
        sticks = [sprout_leaves(b, vals) for b in branches(t)]
...
def add_trees(t1, t2):
    ...
    if len(b1) > len(b2):
        b2 += [tree(0)] * abs(len(b1) - len(b2))
    else:
        b1 += [tree(0)] * abs(len(b1) - len(b2))
