...
class Player(object):
    ...
    def go_to(self, location):
        ...
        print('You are at', str(self.place.name))
        # 两种写法都可以， 用,分割字符串的时候，逗号默认处理为一个空格
    ...
    def take(self, thing):
        ...
        1
        """
        # 这里需要在class Player 的 __init__ 里加入一个 'backpack' attribute
        if type(thing) != str:
            print('Thing should be a string.')
        elif thing in self.place.things:
            take_one = self.place.take(thing)
            self.backpack.append(take_one)
    ...
    def unlock(self, place):
        ...
        if not next_place.locked:
            print(next_place.name, 'is already unlocked!')
        elif key:
            key.use(next_place)
            print(next_place.name, "is now unlocked!")
        else:
            print(next_place.name, "can't be unlocked without a key!")
